  ...
  <div style="width: 92%; margin-top:120px; margin-left:4%;margin-right:4%;">
    <table class="table table-dark table-striped">
      <thead><tr class="table-dark">
        <th style="width:100px; height:auto" class="table-dark">Shop Name</th>
        <th style="width:100px; height:auto" class="table-dark">Address</th>
        <th style="width:100px; height:auto" class="table-dark">Contact</th>
        <th style="width:100px; height:auto" class="table-dark">Bank</th>
        <th style="width:100px; height:auto" class="table-dark">Account</th>
        <th style="width:100px; height:auto" class="table-dark">Purchaser</th>
        <th style="width:100px; height:auto" class="table-dark">Contact</th>
        <th style="width:100px; height:auto" class="table-dark">Address</th>
        <th style="width:100px; height:auto" class="table-dark">Art</th>
...
$query = "SELECT * FROM artwise.art WHERE status='purchasing'";
...
if (mysqli_num_rows($record)>0) {
    while($rec = mysqli_fetch_array($record)){
        ...
        $query1 = 'SELECT * FROM artwise.purchase WHERE product_id="' . $productId . '"';
        $record1 = mysqli_query($connect, $query1);

        if ($record1) {
            if (mysqli_num_rows($record1) > 0) {
                $rec1 = mysqli_fetch_array($record1);
                $sellerId = $rec1['seller_id'];
                $purchaserId = $rec1['purchaser_id'];

                $query2 = 'SELECT * FROM artwise.shop WHERE id="' . $sellerId . '"';
                $record2 = mysqli_query($connect, $query2);

                if ($record2) {
                    if (mysqli_num_rows($record2) > 0) {
                        $rec2 = mysqli_fetch_array($record2);
                        $shopName = $rec2['shop'];
                        $shopAddress = $rec2['address'];
                        $shopContact = $rec2['contact'];
                        $shopBank = $rec2['bank'];
                        $shopAccount = $rec2['acc'];

                        $query3 = 'SELECT * FROM artwise.user WHERE id="' . $purchaserId . '"';
                        $record3 = mysqli_query($connect, $query3);

                        if ($record3) {
                            if (mysqli_num_rows($record3) > 0) {
                                $rec3 = mysqli_fetch_array($record3);
                                $purchaserName = $rec3['name'];
                                $purchaserContact = $rec3['contact'];
                                $purchaserAddress = $rec3['address'];
                              
                                echo
                                    ' <tr class="table-light">
                                    <td style="width:50px; height:auto" class="table-dark">' . $shopName . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $shopAddress . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $shopContact . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $shopBank . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $shopAccount . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $purchaserName . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $purchaserContact . '</td>
                                    <td style="width:50px; height:auto" class="table-dark">' . $purchaserAddress . '</td>
                                    ...
                                    <td style="width:50px; height:auto" class="table-dark"><a href="updateSellStatus.php?pId='.$productId.'" style="background-color: #614AA2; color:white; border-radius: 5px;padding:5px; height: 30px; width: 90px;">Sold</a></td>
                                </tr>';
                              } 
                        }
                    }
            ...
            mysqli_free_result($record3);
        }
        // Reset the result set pointer for the next iteration
        mysqli_data_seek($record1, 0);
    }
    // Free up resources
